...
class gps_calc_class {
 private:
	 ...
	 char csetenv[300];				// Work -- used to set env var GDAL_DATA -- must remain in scope

#if defined(LIBS_GDAL) 
	 OGRSpatialReference *poUTM;						///< Base reference system for the viewer/map -- projected
	 OGRSpatialReference *poLatLong;					///< Base reference system for the viewer/map -- lat/lon
...
#if defined(LIBS_GDAL) 
   OGRSpatialReference* get_spatial_ref();
#endif
};
